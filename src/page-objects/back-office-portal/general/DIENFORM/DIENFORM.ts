...
export class DIENFORM extends BasePage {
    ...
    public async doQuanTam(question, answer, row: number) {
        try {
            let label = By.xpath(`//div[contains(@class, 'geS5n' )  and  .//*[contains(text(),'${question}')]]//div[@role ='group'][${row}]//label[${answer}]`);
            await this.global.waitForSeconds(200);
            let ele = await this.getFieldType(label);
            await ele.click();
            return true
    ...
    public async suAnhHuong(question, answer, row: number) {
        try {
            ...
            let label = By.xpath(`//div[contains(@class, 'geS5n' )  and  .//*[contains(text(),'${question}')]]//div[@data-field-index ='${row}']//div[@class='V4d7Ke'][${answer}]//div[@role='radio']`);
            await this.global.waitForSeconds(200);
            let ele = await this.getFieldType(label);
            await ele.click();
            return true
