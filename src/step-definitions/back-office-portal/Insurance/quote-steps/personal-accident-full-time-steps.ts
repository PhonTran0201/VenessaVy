...
export async function inputPersonalAccidentFullTime(OrganizationNumber: string, CompanyName: string, IndustryCode: string, LossRatio: string, SumInsuredGorNOK: string, SumInsured: string, Claims5Years: string, NumberOfPeople: string) {
...
When("User creates a Personal Accident Full Time quote from csv file {string}", async (filename) => {
...
Then("System shows new Personal Accident Full Time quote in Quote list {string}", async (filename) => {
    const rows = loader(convertPathFileDataToDataRegression(filename));
    let len = rows.length;
...
Then("System shows new Personal Accident Full Time quote in Quote widget at Summary tab {string}", async (filename) => {
    const rows = loader(convertPathFileDataToDataRegression(filename));
    let len = rows.length;
