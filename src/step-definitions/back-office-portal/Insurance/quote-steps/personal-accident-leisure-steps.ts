...
export async function inputPersonalAccidentLeisure(OrganizationNumber: string, CompanyName: string, IndustryCode: string, LossRatio: string, NumberOfPeople: string, Claims5Years: string) {
    ...
    temp = await globalQuoteInsurance.inputNumberOfPeople1(NumberOfPeople);
...
When("User creates a Personal Accident Leisure quote from csv file {string}", async (filename) => {
    ...
    for (let i = 0; i < len; i++) {
        ...
        await inputPersonalAccidentLeisure(
            OrganizationNumber,
            CompanyName,
            IndustryCode,
            LossRatio,
...
Then("System shows new Personal Accident Leisure quote in Quote list {string}", async (filename) => {
    const rows = loader(convertPathFileDataToDataRegression(filename));
    let len = rows.length;
    ...
    for (let i = len - 1, j = 1; i >= 0; i--, j++) {
        ...
        let policyTerm = rows[i].StartDate + " - " + rows[i].EndDate;
...
Then("System shows new Personal Accident Leisure quote in Quote widget at Summary tab {string}", async (filename) => {
    const rows = loader(convertPathFileDataToDataRegression(filename));
    let len = rows.length;
