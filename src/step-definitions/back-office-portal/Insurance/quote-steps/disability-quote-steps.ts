...
export async function inputDisabilityQuote(OrganizationNumber: string, CompanyName: string, IndustryCode: string, LossRatio: string, GroupLifePolicy: string, SumInsured: string, AnotherInsurer: string, Claims5Years: string, EmployeeSSNTAG: string, EmployeeDOBTAG: string) {
    ...
    temp = await globalQuoteInsurance.chooseGroupLifePolicy(GroupLifePolicy);
    logFailTestcase(temp, "choose Group Life Policy failed");

    temp = await globalQuoteInsurance.inputSumInsuredType(SumInsured);
    logFailTestcase(temp, "input Sum Insured failed");

    temp = await globalQuoteInsurance.chooseProductsInsurer(AnotherInsurer);
    logFailTestcase(temp, "choose Another Insurer failed");

    temp = await globalQuoteInsurance.inputClaims5Years(Claims5Years);
    logFailTestcase(temp, "input Claim 5 years failed");

    temp = await globalQuoteInsurance.inputEmployeeSSNTag(EmployeeSSNTAG);
    logFailTestcase(temp, "input EmployeeSSNTag failed");

    temp = await globalQuoteInsurance.inputEmplyeeDOBTag(EmployeeDOBTAG);
    logFailTestcase(temp, "input EmployeeDOBTag failed")
...
When("User creates a Disability quote from csv file {string}", async (filename) => {
    ...
    for (let i = 0; i < len; i++) {
        ...
        const GroupLifePolicy = rows[0].GroupLifePolicy;
        const SumInsured = rows[0].SumInsured;
        const AnotherInsurer = rows[0].AnotherInsurer;
        const Claims5Years = rows[0].Claims5Years;
        const EmployeeSSNTAG = rows[0].EmployeeSSNTAG;
        const EmployeeDOBTAG = rows[0].EmployeeDOBTAG;
...
Then("System shows new Disability quote in Quote list {string}", async (filename) => {
    const rows = loader(convertPathFileDataToDataRegression(filename));
    let len = rows.length;
...
Then("System shows new Disability quote in Quote widget at Summary tab {string}", async (filename) => {
    const rows = loader(convertPathFileDataToDataRegression(filename));
    let len = rows.length;
