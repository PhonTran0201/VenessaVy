...
import { inputPersonalAccidentLeisure } from "../quote-steps/personal-accident-leisure-steps";
...
When("User selectes an existing Personal Accident Leisure quote and create policy from csv file {string}", async (filename) => {
...
When("User selectes an existing Personal Accident Leisure policy and renew policy from csv file {string}", async (filename) => {
  let rows = loader(convertPathFileDataToDataRegression(filename));
...
Then("System shows new policy in Personal Accident Leisure Policy widget at Summary tab {string}", async (filename) => {
