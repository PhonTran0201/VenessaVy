...
import { PaymentOptionFormInsurance } from "../../../../page-objects/back-office-portal/insurance/policy/policy-layout/PaymentOptionFormInsurance";
import { PaymentOptionFormInsuranceVarsam } from "../../../../page-objects/back-office-portal/insurance/policy/policy-layout/PaymentOptionFormInsuranceVarsam";
...
import { inputGroupLifeQuote } from "../quote-steps/group-life-quote-steps";
...
When("User selectes an existing Group Life quote and create policy from csv file {string}", async (filename) => {
...
When("User selectes an existing Group Life policy and renew policy from csv file {string}", async (filename) => {
  let rows = loader(convertPathFileDataToDataRegression(filename));
  ...
  let Employees =
    __dirname.substring(0, temp1.length - 51) + "\\" + rows[0].Employees;


  if (__dirname.includes("jenkins") && !__dirname.includes(".jenkins")) {
    logInfoMessage("Running on jenkins...");
    Employees = __dirname.substring(0, temp1.length - 51) + "/" + rows[0].Employees.replace("\\", "/");
  }
  else {
    logInfoMessage("Runing on local...");
    Employees =
      __dirname.substring(0, temp1.length - 51) + "\\" + rows[0].Employees.replace("/", "\\");
  }
...
Then("System shows new policy in Group Life Policy widget at Summary tab {string}", async (filename) => {
  ...
  for (let i = len - 1, j = 1; i >= 0; i--, j++) {
    let product = rows[i].ProductName || rows[i].SelectedProductName;
    let premium = numberToCurrency(rows[i].PolicyPremium, true);
