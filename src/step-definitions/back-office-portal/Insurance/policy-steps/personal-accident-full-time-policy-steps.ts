...
import { inputPersonalAccidentFullTime } from "../quote-steps/personal-accident-full-time-steps";
...
When("User selectes an existing Personal Accident Full Time quote and create policy from csv file {string}", async (filename) => {
...
When("User selectes an existing Personal Accident Full Time policy and renew policy from csv file {string}", async (filename) => {
  let rows = loader(convertPathFileDataToDataRegression(filename));
  ...
  const SumInsuredGorNOK = rows[0].SuminsuredinGorNOK;
  ...
  await inputPersonalAccidentFullTime(
    OrganizationNumber,
    CompanyName,
    IndustryCode,
    LossRatio,
...
Then("System shows new policy in Personal Accident Full Time Policy widget at Summary tab {string}", async (filename) => {
  ...
  for (let i = len - 1, j = 1; i >= 0; i--, j++) {
    ...
    let premium = rows[i].PolicyPremium;
